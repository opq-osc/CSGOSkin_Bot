...
def get_xhh(url, IP):
    ...
    API = (
        "https://api.xiaoheihe.cn/mall/trade/sku/info?os_type=webinapp&app=heybox&client_type=mobile&version=1.3.231&x_client_type=web&x_os_type=Android&x_app=heybox&sku_id="
        + str(good_id)
    )
...
def zm_login(phone, pwd):
    ...
    data = {"phone": phone, "password": pwd, "remember": 1}
    res = Session.post(zm_url, data=data, headers=headers).json()[
        "ret_data"
    ]  # 登录账号，获取当前SessionID
    bp_data = {
...
class CSGO:
    ...
    def Chk_QQ(self, QQ):
        ...
        """
        cursor = self.sqlc.execute(
            "SELECT *  from ids where QQ=%d" % QQ
        ).fetchone()  # 查询是否存在指定QQ号
        if cursor is not None:
            self.sqlc.execute(
                "UPDATE ids SET LastLogin = %d where QQ=%d" % (int(time.time()), QQ)
            )  # 更新账户最后登录时间
            self.conn.commit()
            return cursor
        else:
    ...
    def Create_QQ(self, QQ):
        ...
        if not self.Chk_QQ(QQ):  # 再次检验账号状态
            self.sqlc.execute(
                "INSERT into ids (QQ, VIP, VipTime, LastLogin) VALUES (%d, 0, -1, %d)"
                % (QQ, int(time.time()))
            )
            self.conn.commit()
            return 1
    ...
    def Ins_Url(self, QQ, url, ExcPrice=-1.0):
        ...
        Info = self.Chk_QQ(QQ)
        flag = 0  # 插入标志符
        lnk_count = self.sqlc.execute(
            "SELECT count(link)  from links where QQ=%d" % QQ
        )  # 统计用户储存链接数
        if Info:  # 查询用户组
            if Info[1] == 0:  # 非会员
        ...
        if flag:
            same_link = self.sqlc.execute(
                "SELECT count(link) from links where QQ=%d and link=%s" % (QQ, Furl)
            )
    ...
    def Del_Url(self, QQ, url):
        flag = self.sqlc.execute(
            "select * from links where QQ=%d and link=%s" % (QQ, "'" + url + "'")
        ).fetchone()
        if flag:
            self.sqlc.execute(
                "Delete from links where link=%s and QQ=%d" % ("'" + url + "'", QQ)
            )
            self.conn.commit()
            return 1
        else:
    ...
    def Create_API(self, QQ):
        ...
        self.sqlc.execute(
            "UPDATE ids set (LastLogin, ChkAPI) = (%d, %s) where QQ = %d"
            % (int(Stamp), Chk_API, QQ)
        )
        self.conn.commit()
    ...
    def Chk_API(self, Code: str):
        ...
        """
        try:
            QQ = self.sqlc.execute(
                "SELECT QQ from ids where ChkAPI = %s" % ("'" + Code + "'")
            ).fetchone()[0]
    ...
    def Get_All_links(self):
        return self.sqlc.execute(
            "SELECT link, NowPrice, Status, QQ, Weapon from links"
        ).fetchall()
