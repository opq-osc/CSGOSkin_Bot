from botoy import FriendMsg, S, Action, jconfig
...
async def receive_friend_msg(ctx: FriendMsg):
	try:
		global action
		if action is None:
			action = Action(jconfig.qq, host=getattr(ctx, "_host"), port=getattr(ctx, "_port"))
		...
		if CMD == '添加':
			if url.isspace() or len(url) == 0 or "[ATALL()]" in url:
				...
				if type(res) == type(abc):
					text = "%s\n上架平台：%s\n当前价格：%s" % (res['name'], res['market'], res['price'])
			...
			Stamp, code = func.sqlc.execute('SELECT LastLogin, ChkAPI from ids where QQ = %d' % ctx.FromUin).fetchone()
			if (time.time() - Stamp >= 600 and code is not None) or code is None:  # 间隔大于十分钟
	...
	if action is not None:
		...
		for res in power:
			if res['flag'] == 0:  # 饰品两次查询均为下架状态
				action.sendFriendText(res['QQ'], '您关注的：%s\n已下架或售出，已在数据库中删除该链接：%s' % (res['name'], res['url'].replace('\"', '')))
				...
				func.sqlc.execute(
					'UPDATE links set ChkTime = %d where QQ = %d and link = %s' % (res['time'], res['QQ'], res['url']))
			elif res['flag'] == 3:  # 更新出售状态但不提示
				func.sqlc.execute(
					'UPDATE links set (ChkTime, Status) = (%d, %d) where QQ = %d and link = %s' % (res['time'], res['status'], res['QQ'], res['url']))
			t += 1
			if t == 50:
				t = 0
				func.conn.commit()
